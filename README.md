...
* **鼠标穿透模式 (New!)**：开启后，鼠标点击操作会直接穿透角色，作用于后方的窗口（适合工作/游戏时挂机）。
* **交互反馈**：支持点击身体不同部位触发不同的动作和语音反馈（仅在非穿透模式下有效）。
* **眼球追踪**：角色的视线和头部会跟随鼠标光标移动。
* **多角色/服装切换**：内置角色库（Azuki, Chocola, Vanilla, Coconut 等）及多种服装配置。
* **系统托盘支持**：
    * 右键菜单：隐藏/显示、锁定位置、**开关鼠标穿透**、退出程序。
    * 双击托盘：快速切换显示/隐藏状态。
* **完全可配置**：支持调整缩放比例 (Scale)、窗口大小及垂直偏移量，并自动保存配置。
...
* **FreeMote-SDK**: 本项目的核心模型驱动与渲染技术使用了 **FreeMote-SDK**。
* **NekoWebShow**: 本项目中关于角色的动作定义、交互逻辑部分代码参考并修改自 **Chocola-X** 的 **NekoWebShow** 项目。
...
├── server.py              # [后端] HTTP 静态文件服务器 (默认端口 5000)
├── config.json            # [配置] 保存用户设置（位置、缩放、锁定、穿透状态）
├── icon.png               # [资源] 系统托盘图标
├── fflate.js              # [库] 用于解压 .zip 格式的模型文件
├── emoteplayer.js         # [驱动] FreeMote SDK 核心播放器
...
将你的 FreeMote 模型文件（`.psb`）放入 `model/` 文件夹，并确保 `config/` 文件夹中有对应的动作配置文件。
*(注：本项目代码中已预设了 Nekopara 系列角色的路径映射，请确保文件名匹配)*
...
* **头部交互无反馈**：目前点击角色的头部区域（Head）时，可能无法正常触发预设的动作或语音反馈。该问题已确认，计划在下一个版本中修复。
...
* **v1.1.0**:
* 新增 **鼠标穿透 (Click Through)** 功能，支持作为纯视觉挂件使用。
* 优化窗口置顶 (Always on Top) 逻辑，防止在切换模式时层级丢失。
* 托盘菜单增加穿透开关，修复了开启穿透后无法操控的问题。
